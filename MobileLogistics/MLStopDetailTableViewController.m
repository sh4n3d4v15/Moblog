...
#import "MLItemTextField.h"
#import "MLClient.h"
#import "TOMSMorphingLabel.h"
@interface MLStopDetailTableViewController () <MKMapViewDelegate,UITextFieldDelegate,UIActionSheetDelegate>
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	return (([shipment.items count] * 70) + 90);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([indexPath section] == 0) {
		...
		[self configureShipmentsView:cell withIndexPath:indexPath];
	}
...
- (void)configureShipmentsView:(UITableViewCell *)cell withIndexPath:(NSIndexPath *)indexPath {
	...
	[[shipment.items allObjects]enumerateObjectsUsingBlock: ^(Item *item, NSUInteger idx, BOOL *stop) {
	    ...
	    MLItemTextField *qtyField  = [[MLItemTextField alloc]initWithFrame:CGRectMake(CGRectGetWidth(shipmentView.bounds) - 60, 10, 50, 30)]; //CVItemTextField
...
- (void)checkMeIn:(id)sender {
	...
	if (!self.stop.actual_arrival) {
		_stop.actual_arrival = [NSDate date];
	}
}
...
-(void)textFieldDidEndEditing:(MLItemTextField *)textField{
